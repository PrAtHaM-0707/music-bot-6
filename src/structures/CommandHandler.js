...
class AvonCommands extends EventEmitter {
  ...
  async run(message) {
    ...
    if (message.content === `<@${this.client.user.id}>`) {
      ...
      let b3 = new ButtonBuilder().setStyle(ButtonStyle.Link).setLabel(`Vote`).setURL(`https://dsc.gg/muzicbot`);
      let ro = new ActionRowBuilder().addComponents(b1, b2, b3);
      let embed = new EmbedBuilder().setColor(this.client.config.color).setFooter({ text: `Made By Habibi ❤️`, iconURL: message.author.displayAvatarURL({ dynamic: true }) }).setDescription(
