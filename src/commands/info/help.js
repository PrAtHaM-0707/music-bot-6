  ...
  async run(client, message, args, prefix) {
    try {
      let em = new EmbedBuilder()
        ...
        .setDescription(`Hey ${message.author} I am ${client.user.username}
             ${client.emoji.arrow} A complete Music Bot for your server
             ${client.emoji.arrow} Providing you the best quality music`)
        .addFields({
          name: `Links`,
          value: `[**Muzic**](https://discord.com/api/oauth2/authorize?client_id=${client.user.id}&permissions=137476565393&scope=bot%20applications.commands) \n [**Support Server**](${client.config.server}) \n [**Vote me**](${client.config.vote})`
        })
        .addFields({
          name : `Command Categories`,
            value : `${client.emoji.filters} \`:\` **Music** \n ${client.emoji.music} \`:\`  **Filters** \n  ${client.emoji.settings} \`:\` **Settings** \n ${client.emoji.info} \`:\`**Information** \n ${client.emoji.allCommands} \`:\` **All Commands**` 
        })
        .setFooter({
          text: `Thanks For Selecting Muzic!`,
      ...
      let select = new SelectMenuBuilder().setCustomId(`ok`).setPlaceholder(`Muzic Is Love!`).addOptions([{
        label: `Help Home`,
